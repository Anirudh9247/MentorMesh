...
export default function MentorCard({ mentor, studentCity }) {
  const { user, domains, bio, avg_rating, session_count } = mentor;
  ...
  return (
    <div className={`relative group p-6 rounded-3xl transition-all duration-300 border ${
      isLocal 
    ...
    } backdrop-blur-xl flex flex-col justify-between h-full`}>
      
      {/* Local Match Badge */}
      {isLocal && (
        <div className="absolute top-4 right-4 flex items-center gap-1 py-1 px-3 rounded-full bg-primary-500/10 border border-primary-500/30 text-primary-400 text-xs font-bold tracking-wide animate-pulse">
      ...
      <div>
        ...
        <div className="flex items-start gap-4 mb-4">
          <div className="w-14 h-14 rounded-2xl bg-gradient-to-tr from-primary-600 to-indigo-500 flex items-center justify-center text-white font-extrabold text-lg shadow-md group-hover:scale-105 transition-transform duration-300">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-slate-400 text-sm mt-1">
              <svg className="w-4 h-4 shrink-0 text-slate-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      ...
      <Link
        ...
        className={`w-full py-3 px-4 rounded-xl font-bold text-sm text-center transition-all duration-300 flex items-center justify-center gap-2 cursor-pointer ${
          isLocal
            ? 'bg-primary-500 hover:bg-primary-400 text-dark-950 shadow-lg shadow-primary-500/20'
